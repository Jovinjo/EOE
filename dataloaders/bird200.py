...
class Cub2011(Dataset):
    ...
    def _load_metadata(self):
        ...
        labels = pd.read_csv(os.path.join(self.root, 'CUB_200_2011', 'image_class_labels.txt'),
                             sep=' ', names=['img_id', 'target'])
        splits = pd.read_csv(os.path.join(self.root, 'CUB_200_2011', 'train_test_split.txt'),
                             sep=' ', names=['img_id', 'is_training_img'])

        data = images.merge(labels, on='img_id').merge(splits, on='img_id')
        self.data = data[data.is_training_img == int(self.train)]
    ...
    def __getitem__(self, idx):
        ...
        target = sample.target - 1  # Shift to 0-based indexing
...
class Cub100(Cub2011):
    ...
    def _load_metadata(self):
        ...
        selected_ids = all_classes[all_classes['target'].isin(self.selected_classes)]['class_id'].tolist()
        remaining_ids = set(all_classes['class_id']) - set(selected_ids)

        if self.id:
            self.data = self.data[self.data['target'].isin(selected_ids)]
            self.class_names_str = [name.split(".")[1].replace('_', ' ') for name in self.selected_classes]
            remaining_names = all_classes[all_classes['class_id'].isin(remaining_ids)]['target'].tolist()
            self.ood_class_name_str = [name.split(".")[1].replace('_', ' ') for name in remaining_names]
        else:
            self.data = self.data[self.data['target'].isin(remaining_ids)]
            remaining_names = all_classes[all_classes['class_id'].isin(remaining_ids)]['target'].tolist()
            self.class_names_str = [name.split(".")[1].replace('_', ' ') for name in remaining_names]
